// <copyright file="OutboundUpdateCase.cs" company="PlaceholderCompany">
...
namespace SiebelDB_001_Jarvis
{
    ...
    /// <summary>
    /// OutboundUpdateCase Azure Function Class.
    /// </summary>
    public class OutboundUpdateCase
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="OutboundUpdateCase"/> class.
        /// </summary>
        /// <param name="config">Configuration.</param>
        /// <param name="dynamicsClient">Dynamics Client.</param>
        public OutboundUpdateCase(IConfiguration config, IDynamicsApiClient dynamicsClient)
        {
            this.config = config;
            this.dynamicsClient = dynamicsClient;
            this.azureServicebusConnectionstring = this.config.GetSection("ServiceBusConnection").Value;
            this.queueName = this.config.GetSection("OutboundUpdateCaseQueue").Value;
        }
        ...
        [FunctionName("outboundupdatecase")]
            ...
            ILogger log)
        {
            ...
            if (!string.IsNullOrEmpty(incidentId) && !string.IsNullOrEmpty(transType))
            {
                ...
                UpdateCasesOut casesOut = new UpdateCasesOut(this.dynamicsClient, this.config, logger);
