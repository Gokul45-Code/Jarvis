// <copyright file="AddCaseMonitorActionInbound.cs" company="Microsoft">
...
namespace IntegrationProcess
{
    ...
    /// <summary>
    /// Add Case Monitor Action Inbound.
    /// </summary>
    public class AddCaseMonitorActionInbound
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AddCaseMonitorActionInbound"/> class.
       /// </summary>
        /// <param name="dynamicsApiClient">Dynamic Client.</param>
        /// <param name="logger">logger.</param>
        public AddCaseMonitorActionInbound(IDynamicsApiClient dynamicsApiClient, ILoggerService logger)
        {
            ...
            try
            {
                ...
                if (payLoad != null && !string.IsNullOrEmpty(caseNumberArgus))
                {
                              ...
                              && configRecord.First().ToObject<JObject>().TryGetValue(Constants.JarvisIntegrationMapping, StringComparison.OrdinalIgnoreCase, out JToken? intMapping))
                    {
                        ...
                        if (jarvis_congif != null && jarvis_congif.TryGetValue(SiebelConstants.Casemonitoraction, StringComparison.OrdinalIgnoreCase, out JToken? caseconfigMapping))
                        {
...
#pragma warning restore S1481 // Unused local variables should be removed
                        ...
                        throw new ArgumentException("Payload does not contain configuration unique indentifier");
                    }
                    else
                    {
                        this.logger.LogException(new ArgumentException("No Integration Configuration Found/ Integration is not Active, Pleae activate the configuraiton record."));
        ...
        private (int, string) ServiceRequestBdArgusToCase(JObject currentPayload, JObject? mappings, string incidentid)
        {
            try
            {
                ...
                if (mappings != null && mappings.TryGetValue(Constants.JarvisFieldMappings, out JToken? fieldMapping) && fieldMapping != null)
                {
                    ...
                    payload.Add(SiebelConstants.CaseMonitorRegardingObject, string.Format("/{0}({1})", Constants.Incidents, incidentid));
                    if (payload.SelectToken(SiebelConstants.JarvisFollowUpTime) == null || string.IsNullOrEmpty(payload.SelectToken(SiebelConstants.JarvisFollowUpTime)?.ToString()))
                    {
                        payload.Add(SiebelConstants.PriorityCode, PriorityCode.High);
                        TimeZoneInfo gmtPlus1TimeZone = TimeZoneInfo.FindSystemTimeZoneById(Constants.GmtPlus1TimeZone);
                        DateTime currentDateTime = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, gmtPlus1TimeZone);

                        // if (payload.SelectToken(SiebelConstants.ActualstartDate) != null)
                        // {
                        //    JToken existingValue = payload.SelectToken(SiebelConstants.ActualstartDate);
                        //    existingValue.Replace(currentDateTime.Date);
                        // }
                        if (payload.SelectToken(SiebelConstants.JarvisFollowUpTime) != null)
                        {
                            JToken? existingValue = payload.SelectToken(SiebelConstants.JarvisFollowUpTime);
                            existingValue?.Replace(currentDateTime.ToString("HH:mm", CultureInfo.InvariantCulture));
                        }
                    }
                    else if (Convert.ToBoolean(payload?.SelectToken(SiebelConstants.JarvisSa)?.ToString()))
                    {
                        payload.Add(SiebelConstants.PriorityCode, PriorityCode.Medium);
                    }
                    else
                    {
                        payload?.Add(SiebelConstants.PriorityCode, PriorityCode.Standard);
                    }
                }
                else
                {
        ...
        private async Task GetConfigMasterDataAsBatch(string caseNumberArgus, string caseNumberJarvis)
        {
            try
            {
                ...
                masterContents.Add(DynamicsApiClient.CreateHttpMessageContent(HttpMethod.Get, Constants.JarvisIntegrationConfiguration, string.Format(CrmQueries.JarvisIntegrationConfigurationQuery, "CASEMONITORACTION001".ToString().ToUpper()), 1, string.Empty, false));
                ...
                {
                    this.logger.LogException(new ArgumentException("Getting IntegrationConfiguration,MasterData and Incident record Failed"));
