// <copyright file="AddJobEndDetailsInbound.cs" company="Microsoft">
...
namespace IntegrationProcess
{
    ...
    /// <summary>
    /// AddJobEndDetailsInbound.
    /// </summary>
    public class AddJobEndDetailsInbound
    {
        /// <summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="AddJobEndDetailsInbound"/> class.
       /// </summary>
        /// <param name="dynamicsApiClient">Dynamics Clients.</param>
        /// <param name="logger">Logger.</param>
        public AddJobEndDetailsInbound(IDynamicsApiClient dynamicsApiClient, ILoggerService logger)
        {
        ...
        public async Task<HttpResponseMessage> IntegrationProcessAsync(JObject payLoad, string caseNumberArgus, string caseNumberJarvis, string transType, DateTime eventTimestamp)
        {
            try
            {
                ...
                {
                    string? type = payLoad.SelectToken(SiebelConstants.JobEndDetailType)?.ToString();
                    string? bpResponsibleUnit = payLoad.SelectToken(SiebelConstants.JobEndResponsibleUnitId)?.ToString();
                    string? bpRetailCountry = payLoad.SelectToken(SiebelConstants.JobEndRetailCountryId)?.ToString();
                              ...
                              && configRecord.First().ToObject<JObject>().TryGetValue(Constants.JarvisIntegrationMapping, StringComparison.OrdinalIgnoreCase, out JToken? intMapping))
                    {
                        ...
                        if (jarvis_congif != null && jarvis_congif.TryGetValue(transType, StringComparison.OrdinalIgnoreCase, out JToken? caseconfigMapping) && type != null &&
                            caseconfigMapping.ToObject<JObject>().TryGetValue(type.ToString(), StringComparison.OrdinalIgnoreCase, out JToken? jobEndInfoconfigMapping))
                        {
                            this.helper.ValidateSetFieldMapping(payLoad, jobEndInfoconfigMapping, this.retrieveList);
                            if (transType.ToUpper() == SiebelConstants.JobEnd1.ToUpper())
                            {
                                //// ServiceRequestBdArgus to Incident Reapir Info Payload generation.
#pragma warning disable S1481 // Unused local variables should be removed
                                var (contentId, targetEntity) = this.ServiceRequestBdArgusToCase(payLoad, jobEndInfoconfigMapping.ToObject<JObject>(), incidentid, passoutId, type.ToString(), eventTimestamp);
...
#pragma warning disable S1481 // Unused local variables should be removed
                                var (contentId, targetEntity) = this.ServiceRequestBdArgusToTranslation(payLoad, jobEndInfoconfigMapping.ToObject<JObject>(), incidentid);
#pragma warning restore S1481 // Unused local variables should be removed
                            }
        ...
        /// <returns>Task.</returns>
        /// <exception cref="ArgumentException">Argument Exception.</exception>
        private (int, string) ServiceRequestBdArgusToCase(JObject currentPayload, JObject? mappings, string incidentid, string passoutId, string type, DateTime eventTimestamp)
        {
            try
            {
                JObject payload = new ();
                ...
                string? targetLookupEntity = DynamicsApiHelper.GetStringValueFromJObject(mappings, Constants.TargetLookupEntity).ToLower();
                ...
                var retrieveJobEndDetails = this.LookupValueJobEndDetail(this.retrieveList, currentPayload, sourceField, targetLookupEntity, targetLookupEntityName, targetLookupFieldValue, passoutId);

                string recordkey = string.Empty;
                if (retrieveJobEndDetails.Item1 && retrieveJobEndDetails.value != null)
                {
#pragma warning disable S1854 // Unused assignments should be removed
                    recordkey = retrieveJobEndDetails.value.ToString();
                    var jobEndItems = this.retrieveList.First(item => item.Key.ToUpper().Contains(SiebelConstants.JarvisJobenddetailses.ToUpper())).Value;
                    var jobEnd = jobEndItems?.FirstOrDefault(x => x[targetLookupEntityName]?.ToString() == recordkey);
                    var lastUpdateTimeStamp = jobEnd?.SelectToken(SiebelConstants.JarvisJobendupdatetimestamp);
                    if (lastUpdateTimeStamp != null && !string.IsNullOrEmpty(lastUpdateTimeStamp.ToString()))
                    {
                        ...
                        if (result <= 0)
                        {
                            this.logger.LogException(new Exception("Payload Timesamp should be greater than Job End Detail's Timestamp in Jarvis"));
                            throw new ArgumentException("Payload Timestamp should be greater than Job End Detail's Timestamp in Jarvis");
                        }
                    }
#pragma warning restore S1854 // Unused assignments should be removed
                }
                else if (!retrieveJobEndDetails.Item1 && retrieveJobEndDetails.value != null)
                {
                    this.isCreate = true;
                ...
                if (mappings != null && mappings.TryGetValue(Constants.JarvisFieldMappings, out JToken? fieldMapping) && fieldMapping != null)
                {
                    ...
                    payload.Add(SiebelConstants.JarvisJobendupdatetimestamp, eventTimestamp);
                    if (this.isCreate)
                    {
                        ...
                        payload.Add(Constants.Statuscode, 334030001);
                        ////if (type.ToUpper() == SiebelConstants.jobEndActualCasuseFault.ToUpper())
                        ////{
                        ////    payload.Add(SiebelConstants.jarvis_temporaryrepair, "Not required");
                        ////}
                        ////else if (type.ToUpper() == SiebelConstants.jobEndTempRepair.ToUpper())
                        ////{
                        ////    payload.Add(SiebelConstants.jarvis_actualcausefault, "Not required");
                        ////}
                    }
                }
                else
        ...
        private (int, string) ServiceRequestBdArgusToTranslation(JObject currentPayload, JObject? mappings, string incidentid)
        {
            try
            {
                ...
                string jobEndDetailId = string.Empty;
                string? sourceField = DynamicsApiHelper.GetStringValueFromJObject(mappings, Constants.SourceFieldSchema);
...
#pragma warning disable S1854 // Unused assignments should be removed
                    ...
                    jobEndDetailId = retrieveCaseTranslation.value.ToString();
#pragma warning restore S1854 // Unused assignments should be removed
                }
                ...
                if (mappings != null && mappings.TryGetValue(Constants.JarvisFieldMappings, out JToken? fieldMapping) && fieldMapping != null)
                {
                    ...
                    if (this.isCreate)
                    {
                        ...
                        payload.Add(SiebelConstants.JarvisJobEndDetailsBind, string.Format("/{0}({1})", SiebelConstants.JarvisJobenddetailses, jobEndDetailId));
        ...
        private (bool, JToken? value) LookupValueTranslation(Dictionary<string, JArray?> retrieveList, JObject sourcePayload, string sourceField, string targetLookupEntity, string targetEntityName, string targetAlternateKeys, string targetLookupFieldValue)
        {
            try
            {
                ...
                var jodEndDetailList = matchMasterRecord?.Where(item => passoutRecord.Select(item => item[SiebelConstants.JarvisPassoutid]?.ToString()).Contains(item[SiebelConstants.JarvisRepairingdealerpassoutValue]?.ToString()));
                if (!string.IsNullOrEmpty(sourceField) && !string.IsNullOrEmpty(targetLookupEntity) && !string.IsNullOrEmpty(targetEntityName) && matchMasterRecord != null && jodEndDetailList?.ToList().Count > 0)
                {
                    var jobEndTranslation = jodEndDetailList.SelectMany(item => item.SelectToken(targetAlternateKeys));
                    var sourceValue = sourcePayload.SelectToken(sourceField)?.Value<JToken>();
                    if (!string.IsNullOrEmpty(sourceValue?.ToString()))
                    {
                        var matchRecord = jobEndTranslation.Where(item => item.SelectToken(targetLookupFieldValue)?.ToString().ToUpper() == sourceValue.ToString().ToUpper() && item[Constants.JarvisType]?.ToString() == "334030001").OrderByDescending(item => item["modifiedon"]).ToList();
                        ...
                        {
                            JToken? jToken = jodEndDetailList.Select(item => item[SiebelConstants.JarvisJobenddetailsid]).First();
                            return (false, jToken);
                        }
                    }
                }
        ...
        private (bool, JToken? value) LookupValueJobEndDetail(Dictionary<string, JArray?> retrieveList, JObject sourcePayload, string sourceField, string targetLookupEntity, string targetEntityName, string targetLookupFieldValue, string passoutId)
        {
            try
            {
                var matchMasterRecord = retrieveList.First(item => item.Key.ToUpper().Contains(targetLookupEntity.ToUpper())).Value;
                var jodEndDetailList = matchMasterRecord?.Where(item => item[SiebelConstants.JarvisRepairingdealerpassoutValue]?.ToString() == passoutId);
                if (!string.IsNullOrEmpty(sourceField) && !string.IsNullOrEmpty(targetLookupEntity) && !string.IsNullOrEmpty(targetEntityName))
                {
                    var sourceValue = sourcePayload.SelectToken(sourceField)?.Value<JToken>();
                    if (!string.IsNullOrEmpty(sourceValue?.ToString()))
                    {
                        var matchRecord = jodEndDetailList?.Where(item => item.SelectToken(targetLookupFieldValue)?.ToString().ToUpper() == sourceValue.ToString().ToUpper()).OrderByDescending(item => item["modifiedon"]).ToList();
        ...
        private async Task GetConfigMasterDataAsBatch(string caseNumberArgus, string caseNumberJarvis, string transType, string? bpResponsibleUnit, string? bpRetailCountry)
        {
            try
            {
                ...
                masterContents.Add(DynamicsApiClient.CreateHttpMessageContent(HttpMethod.Get, Constants.JarvisIntegrationConfiguration, string.Format(CrmQueries.JarvisIntegrationConfigurationQuery, "JOBENDDETAIL001".ToString().ToUpper()), 1, string.Empty, false));
                ...
                if (transType.ToUpper() == SiebelConstants.JobEnd2.ToUpper())
                {
                    masterContents.Add(DynamicsApiClient.CreateHttpMessageContent(HttpMethod.Get, SiebelConstants.JarvisJobenddetailses, string.Format(CrmQueries.JobEndDetailsWithTranslation, caseNumberArgus, caseNumberJarvis), 1, string.Empty, false));
                ...
                {
                    masterContents.Add(DynamicsApiClient.CreateHttpMessageContent(HttpMethod.Get, SiebelConstants.JarvisJobenddetailses, string.Format(CrmQueries.JobEndDetails, caseNumberArgus, caseNumberJarvis), 1, string.Empty, false));
