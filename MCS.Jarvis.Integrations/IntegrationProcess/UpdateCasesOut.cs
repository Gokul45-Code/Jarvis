// <copyright file="UpdateCasesOut.cs" company="Microsoft">
...
namespace IntegrationProcess
{
    ...
    public class UpdateCasesOut
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="UpdateCasesOut"/> class.
        /// UpdateCasesOut Constructor.
        /// </summary>
        /// <param name="dynamicsApiClient">DynamicsApiClient.</param>
        /// <param name="config">Config.</param>
        /// <param name="logger">Logger.</param>
        public UpdateCasesOut(IDynamicsApiClient dynamicsApiClient, IConfiguration config, ILoggerService logger)
        {
...
#pragma warning restore S1481 // Unused local variables should be removed
        ...
        private async Task GetConfigMasterDataAsBatch(string sourceCaseId, string? modifiedBy, string? caseWorker)
        {
            try
            {
                ...
                masterContents.Add(DynamicsApiClient.CreateHttpMessageContent(HttpMethod.Get, Constants.JarvisIntegrationConfiguration, string.Format(CrmQueries.JarvisIntegrationConfigurationQuery, "MERCURIRUS002".ToString().ToUpper()), 1, string.Empty, false));
            ...
            try
            {
                ...
                if (matchMasterRecord != null && matchMasterRecord.Count > 0)
                {
                    ...
                    if (mappings != null && mappings.TryGetValue(Constants.JarvisFieldMappings, out JToken? fieldMapping) && fieldMapping != null && payload != null && payload.HasValues)
                    {
                        ...
                        ////RequestWorkerLoginId/RequestWorkerBDARGUS
                        this.logger.LogTrace($"RequestWorkerLoginId: {casePayload.SelectToken(SiebelConstants.RequestWorkerLoginId) != null}, {casePayload.SelectToken(SiebelConstants.RequestWorkerLoginId)}");
                        var userRecords = this.retrieveList.First(item => item.Key.ToUpper().Contains(Constants.Users.ToUpper())).Value?.ToList();
                        if (userRecords != null && userRecords.Count > 0 && caseWorker != null)
                        {
                            var caseWorkerRecord = userRecords.First(item => item[Constants.Systemuserid]?.ToString().ToLower() == caseWorker.ToLower()).ToObject<JObject>();
                            this.logger.LogTrace($"OwningUserFullName: {caseWorkerRecord != null}");
                            if (caseWorkerRecord != null && caseWorkerRecord.SelectToken(Constants.Fullname) != null && casePayload.SelectToken(SiebelConstants.RequestWorkerLoginId) != null)
                            {
                                this.logger.LogTrace($"OwningUserFullName: {caseWorkerRecord.SelectToken(Constants.Fullname)}");
                                if (caseWorkerRecord.SelectToken(Constants.Fullname).ToString().ToUpper().Contains(SiebelConstants.Mercurius))
                                {
                                    casePayload.SelectToken(SiebelConstants.RequestWorkerLoginId)?.Replace(SiebelConstants.Mercurius);
                                }
                                else if (caseWorkerRecord.SelectToken(Constants.Fullname).ToString().ToUpper().Contains(SiebelConstants.UnAssigned) || casePayload.ContainsKey(SiebelConstants.ForcedClosedARGUS))
                                {
                                    casePayload.SelectToken(SiebelConstants.RequestWorkerLoginId)?.Replace(null);
                                    casePayload.SelectToken(SiebelConstants.RequestWorkerBDARGUS)?.Replace(null);
                                }
                            }
                        }
                    }
